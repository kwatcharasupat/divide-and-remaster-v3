...
def get_data_list(data_source, cfg):
    ...
    df["variant"] = data_source.variant
...
def make_manifest_and_splits(cfg):
    ...
    print(speaker_ids[["gender", "variant"]].value_counts())
    ...
    # manually put es-es into test
    test_speaker_ids_ = valtest_speaker_ids[valtest_speaker_ids["variant"] == "es-es"]

    valtest_speaker_ids = valtest_speaker_ids[valtest_speaker_ids["variant"] != "es-es"]
...
def organize_split(manifest, split, cfg):
    ...
    manifest["langcode"] = "spa"
    manifest["language"] = "Spanish"
