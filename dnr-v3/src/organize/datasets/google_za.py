...
def get_data_list(data_source, cfg):
    ...
    df["variant"] = df["language"].copy()

    df["speaker_id"], df["track_id"] = zip(
        *df["file"].apply(file_name_to_speaker_track)
    )

    df["speaker_id"] = df.apply(lambda x: f'{x["variant"]}-{x["speaker_id"]}', axis=1)
...
def make_manifest_and_splits(cfg):
    ...
    speaker_ids = manifest[["speaker_id", "language"]].drop_duplicates()

    print(f"Found {len(unique_speakers)} unique speakers")

    for lang, dfg in manifest.groupby("language"):
        print(f"{lang}: {len(dfg)} tracks")
        print(f"{lang}: {len(dfg['speaker_id'].unique())} speakers")
        print(dfg.value_counts(["speaker_id", "variant"]).sort_index())

    # print(speaker_ids)

    assert len(unique_speakers) == len(speaker_ids), "Speaker IDs are not unique"

    stratifier_cols = ["language"]

    speaker_stratifier = speaker_ids[stratifier_cols].values

    print(speaker_ids[stratifier_cols].value_counts())

    train_speaker_ids, valtest_speaker_ids = train_test_split(
        speaker_ids, test_size=0.3, random_state=cfg.seed, stratify=speaker_stratifier
    )

    valtest_speaker_gender_variants = valtest_speaker_ids[stratifier_cols].values
    ...
    vc = manifest.groupby(["split"]).value_counts(["variant"]).sort_index()
...
def organize_file(file, langcode, variant, speaker_id, track_id, split, cfg):
    ...
    metadata = {
        ...
        "langcode": langcode,
        "language": LANGCODE_TO_LANGUAGE[langcode],
        "subset": subset,
